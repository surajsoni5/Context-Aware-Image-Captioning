...
def beam_search(encoder, decoder, image_path, beam_size=3):
    """
    ...
    """
    #FIXME: set eval for encoder and decoder
...
def beam_search_justify(encoder, decoder, image_path, beam_size,class_k):
    """
    Reads an image and captions it with beam search.
    ...
    transform = transforms.Compose([normalize])
    image = transform(img)  # (3, 256, 256)
    class_embedding=decoder.class_embedding(torch.LongTensor([[class_k]]).to(device))
    # Encode
    image = image.unsqueeze(0)  # (1, 3, 256, 256)
    encoder_out = encoder(image)  # (1, enc_image_size, enc_image_size, encoder_dim)
    enc_image_size = encoder_out.size(1)
    ...
    # We'll treat the problem as having a batch size of k
    encoder_out = encoder_out.expand(k, num_pixels, encoder_dim)  # (k, num_pixels, encoder_dim)
    class_embedding=class_embedding.expand(k,1,512) ####FIXME: Check
    # Tensor to store top k previous words at each step; now they're just <start>
    k_prev_words = torch.LongTensor([[start]] * k).to(device)  # (k, 1)
    ...
    # Start decoding
    step = 1
    h, c = decoder.init_hidden_state(encoder_out,class_embedding)
    ...
    while True:
        ...
        gate = decoder.sigmoid(decoder.f_beta(h))  # gating scalar, (s, encoder_dim)
        awe = gate * awe
        h, c = decoder.decode_step(torch.cat([embeddings, awe,class_embedding[:,0,:]], dim=1), (h, c))  # (s, decoder_dim)
        ...
        top_k_scores = top_k_scores[incomplete_inds].unsqueeze(1)
        k_prev_words = next_word_inds[incomplete_inds].unsqueeze(1)
        class_embedding=class_embedding[incomplete_inds,:,:]
...
def beam_search_justify_main(encoder, decoder, image_path ,class_t ,class_d,lambda_, beam_size):
    ...
    # We'll treat the problem as having a batch size of k
    encoder_out = encoder_out.expand(k, num_pixels, encoder_dim)  # (k, num_pixels, encoder_dim)
    class_embedding_t=class_embedding_t.expand(k,1,512) ####FIXME: Check
    class_embedding_d=class_embedding_d.expand(k,1,512) ####FIXME: Check
    # Tensor to store top k previous words at each step; now they're just <start>
    k_prev_words = torch.LongTensor([[start]] * k).to(device)  # (k, 1)
    ...
    while True:
        ...
        awe_t, alpha_t = decoder.attention(encoder_out, h_t)  # (s, encoder_dim), (s, num_pixels)
        awe_d, alpha_d = decoder.attention(encoder_out, h_d)  # (s, encoder_dim), (s, num_pixels)
...
if __name__ == "__main__":
    if sys.argv[1]=="caption": ## Caption image_path 
        ...
        seq, alphas=beam_search(encoder,decoder,image_path,20)
        for i in seq[1:]:
            print(word_map[i].decode("utf-8") ,end=" ")
        print("")
    elif sys.argv[1]=="caption_justify": ## Caption Image_path class
        checkpoints=torch.load('checkpoint_justify')
        encoder=checkpoints['encoder']
        decoder=checkpoints['decoder']
        encoder.eval()
        decoder.eval()
        image_path="Rufous_Hummingbird_0009_59405.jpg"
        word_map=tf.load('C:/Users/hello/Desktop/Accads/cvpr2016_cub/vocab_c10.t7',force_8bytes_long=True)
        word_map={word_map[i]:i for i in word_map}
        seq, alphas=beam_search_justify_main(encoder,decoder,image_path,69,68,0.2,2)
        for i in seq[1:]:
            print(word_map[i].decode("utf-8") ,end=" ")
        print("")
    else:
